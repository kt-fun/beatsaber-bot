...
export default function schedules(ctx:Context,config:Config) {
  ...
  ScoreMonitor(ctx,config,render,api,ctx.logger('bsbot.schedules.beatsaver.scoreMonitor'))
  ctx.cron(config.bsNotifyMonitorCron,beatSaverAlertMonitor(ctx,config,render,api,ctx.logger('bsbot.schedules.beatsaver.alertMonitor')))

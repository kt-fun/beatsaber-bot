...
export const handleScoreSaberBind = async (
  ctx: Context,
  api: APIService,
  {
    session,
    options,
  }: { session: Session<'id', never, Context>; options: object },
  input: string
) => {
  const scoreSaberUser = await api.ScoreSaber.wrapperResult()
    .withRetry(3)
    .getScoreUserById(input)
  if (!scoreSaberUser.isSuccess()) {
    const text = session.text('commands.bsbot.bind.not-found', {
      id: input,
      platform: 'ScoreSaber',
    })
  ...
  const { ssAccount, blAccount } = await getUserBSAccountInfo(
    ctx,
    session.user.id
  )

  const text =
    session.text('commands.bsbot.bind.ack-prompt', {
      user: `${scoreSaberUser.data.name}(${scoreSaberUser.data.id})`,
    }) +
    (ssAccount
      ? ',' +
        session.text('commands.bsbot.bind.exist', {
  ...
  const now = new Date()
  const account: Partial<BSRelateOAuthAccount> = {
    uid: session.user.id,
    platform: 'scoresaber',
  ...
  const res = await ctx.database.upsert('BSRelateOAuthAccount', [account])
  session.sendQuote(
    session.text('commands.bsbot.bind.success', {
      user: `${scoreSaberUser.data.name}(${scoreSaberUser.data.id})`,
    })
  )
}

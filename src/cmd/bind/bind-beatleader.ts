...
export const handleBeatLeaderBind = async (
  ctx: Context,
  api: APIService,
  {
    session,
  ...
  const tokenInfo = await api.AIOSaber.wrapperResult().getBLOAuthToken(input)
  if (!tokenInfo.isSuccess()) {
    session.sendQuote(session.text('commands.bsbot.bind.bl.not-found'))
    return
  }
  let token = tokenInfo.data
  let self = await api.BeatLeader.wrapperResult().getTokenInfo(
    token.access_token
  )
  if (!self.isSuccess()) {
    const refreshToken = await api.BeatLeader.wrapperResult().refreshOAuthToken(
      token.refresh_token
    )
    ...
    self = await api.BeatLeader.withRetry(3)
      .wrapperResult()
  ...
  session.sendQuote(
    session.text('commands.bsbot.bind.bl.success', {
      name: self.data.name,
      id: self.data.id,
    })
  )

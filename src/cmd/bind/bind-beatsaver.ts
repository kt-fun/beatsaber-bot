...
export const handleBeatSaverBind = async (
  ctx: Context,
  api: APIService,
  {
    session,
  ...
  const tokenInfo = await api.AIOSaber.wrapperResult()
    .withRetry(3)
  ...
  let self = await api.BeatSaver.wrapperResult().getTokenInfo(
    token.access_token
  )
  if (!self.isSuccess()) {
    const refreshToken = await api.BeatSaver.wrapperResult().refreshOAuthToken(
      token.refresh_token
    )
    ...
    self = await api.BeatSaver.wrapperResult()
      .withRetry(3)
  ...
  session.sendQuote(
    session.text('commands.bsbot.bind.bs.success', {
      name: self.data.name,
      id: self.data.id,
    })
  )

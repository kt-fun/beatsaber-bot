...
export function MeCmd(
  ...
  const meCmd = ctx
    ...
    .action(async ({ session, options }, input) => {
      const rankPlatform = options.p == 'ss' ? Platform.SS : Platform.BL
      const { blAccount, ssAccount } = await getUserBSAccountInfo(
        ctx,
        session.user.id
      )
      let accountId
      ...
      if (input && input !== '') {
        ...
        if (!scoreReq.isSuccess()) {
          return session.text('commands.bsbot.me.score-not-found', {
            user: accountId,
            id: input,
          })
        }
        ...
        const img = await render.renderRank(
          accountId,
          rankPlatform,
          onStartRender
        )

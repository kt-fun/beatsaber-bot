  ...
  const serachCmd = ctx
    ...
    .action(async ({ session, options }, input) => {
      ...
      if (input.length > 15) {
        ...
        session.sendQuote(
          session.text('commands.bsbot.search.too-long-key', { key })
        )
      }
      const res = await api.BeatSaver.wrapperResult().searchMapByKeyword(key)
      if (!res.isSuccess()) {
        session.sendQuote(
          session.text('commands.bsbot.search.not-found', { key })
        )
        return
      }
      const onStartRender = () => {
      ...
      const text = session.text('commands.bsbot.search.success', {
        key: key,

...
export const bsRequest = (ctx: Context, cfg: Config) => {
  ...
  const searchMapByKeyword = async (key: string) =>
    http
      .get<BSMapLatestResponse>(url(`/search/text/0?q=${key}`))
      .then((res) => res.docs)

  const searchMapById = async (id: string) =>
    http.get<BSMap>(url(`/maps/id/${id}`))

  const getAlertsStats = async (accessToken: string) =>
    http.get<BeatsaverAlertStats>(url(`/alerts/stats`), {
  ...
  const getAlertsByPage = async (
    ...
    http.get<BeatsaverAlert[]>(url(`/alerts/${type}/${page}`), {
      headers: { Authorization: `Bearer ${accessToken}` },
    })

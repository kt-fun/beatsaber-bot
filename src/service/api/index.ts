...
type ServiceWithHelper<API, Wrap extends boolean = false> = Wrapped<API,Wrap> & {
  helper: APIHelper<API,Wrap>
}

type Wrapping<T, I extends boolean = false> = T extends ((...args: infer Args) => infer R) ?
  <Wrap extends boolean = false>(...args: Args) => (
    R extends Promise<infer R1>  ?
      (
        I extends true ? Promise<NetReqResult<R1>> :
          (Wrap extends true ? Promise<NetReqResult<R1>> : Promise<R1>)
      )
      : R
    )
  : T;
...
function createProxiedAPIService<T extends Object>(instance: T): ServiceWithAPIHelper<T> {
  ...
  return new Proxy<T>(instance, {
    get(target, prop, receiver) {
      ...
      if (typeof method === 'function') {
       return  async function (...args: any[]) {
          const result = await (target as ServiceWithHelper<T>).helper.call(() =>  method.apply(this, args));
          return result;
        };
      }
      if (prop in target) {
    ...
    set(target, property, value, receiver) {
      ...
      if (property in (target as ServiceWithHelper<T>).helper) {
        return Reflect.set((target as ServiceWithHelper<T>).helper, property, value);
      }
      return false;
    }
...
export class APIService {
  ...
  private constructor(ctx:Context, cfg: Config) {
    ...
    this.BeatLeader = new BeatLeaderService(bsClient, blClient) as unknown  as ServiceWithAPIHelper<BeatLeaderService>
    this.BeatSaver = new BeatSaverService(bsClient)  as unknown as ServiceWithAPIHelper<BeatSaverService>
    this.ScoreSaber = new ScoreSaberService(bsClient, scClient)  as unknown as ServiceWithAPIHelper<ScoreSaberService>
    this.AIOSaber = new AIOSaberService(aioClient)  as unknown as ServiceWithAPIHelper<AIOSaberService>
  }

  static create(ctx:Context, cfg:Config) {
    let ins = new APIService(ctx, cfg)
  ...
  async call<T>(block:()=> Promise<T>):Promise<Res<Base, T>> {
      ...
      while (times > 0) {
        try {
          ...
          await sleep(300)
          if(wrap) {
            return NetReqResult.failed<T>(`retry times reached, ${e}`) as Res<Base, T>
          }

...
export class RenderService {
  ...
  async renderRank(accountId: string, platform:Platform,  onRenderStart?: ()=>void, onRenderError?: (e)=>void,type:'remote' | 'local' = this.config.renderMode) {
    return this._renderRank(accountId, platform, this.api, {...this.baseConfig, type, onRenderError, onRenderStart} as RenderOption);
  }

  private _renderRank = async (accountId: string, platform: Platform, api:APIService, renderOpts:RenderOption) => {
    ...
    if(platform == Platform.BL) {
      ...
      return renderBLPlayerImg(renderOpts.puppeteer, scores, userInfo, renderOpts.onRenderStart, renderOpts.onRenderError)
    }
    const {scores, userInfo}  = await api.ScoreSaber.getPlayerRecentScoreWithUserInfo(accountId)
    return renderSSPlayerImg(renderOpts.puppeteer, scores, userInfo, renderOpts.onRenderStart, renderOpts.onRenderError)
  }

  async renderScore(scoreId: string, platform: Platform, onRenderStart?: ()=>void, onRenderError?: (e)=>void, type:'remote' | 'local' = this.config.renderMode) {
    return this._renderScore(scoreId, platform, this.api, {...this.baseConfig, type, onRenderError, onRenderStart} as RenderOption);
  }

  private _renderScore = async (scoreId: string, platform: Platform, api:APIService, renderOpts:RenderOption) => {
    ...
    return renderBLScoreImg(renderOpts.puppeteer, score, bsMap, statistic, bsor, qrcodeUrl, renderOpts.onRenderStart, renderOpts.onRenderError)
  }


  async renderMapById(mapId: string, onRenderStart?: ()=>void, onRenderError?: (e)=>void, type:'remote' | 'local' = this.config.renderMode) {
    const map = await this.api.BeatSaver.searchMapById(mapId)
    return this._renderMap(map, {...this.baseConfig, type, onRenderError, onRenderStart} as RenderOption);
  }

  async renderMap(map: BSMap, onRenderStart: ()=>void = noop, onRenderError: (e)=>void = noop, type:'remote' | 'local' = this.config.renderMode) {
    return this._renderMap(map, {...this.baseConfig, type, onRenderError, onRenderStart} as RenderOption);
  }

  async renderLBScore() {
   return  screenshotTmp(this.puppeteer, 'https://aiobs.ktlab.io/tmp/lb/score', '#render-result',()=>{}, 8000)
  }

  async renderLBHitCount() {
    return  screenshotTmp(this.puppeteer, 'https://aiobs.ktlab.io/tmp/lb/hitcnt', '#render-result',()=>{}, 8000)
  }
  ...
  private _renderMap = async (bsMap: BSMap, renderOpts:RenderOption) => {
    ...
    return await renderBSMapImg(renderOpts.puppeteer,bsMap, bsMapQrUrl, previewQrUrl, renderOpts.onRenderStart, renderOpts.onRenderError)
  }
}

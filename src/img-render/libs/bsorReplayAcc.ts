import { BSOR, BSORNote, NoteCutInfo } from "../interface/beatleaderScore";
...
export function processAccGraphs(replay:BSOR) {
	...
	let result = {
		times: [] as any[],
		fullSwingBySaber: {red: []as any[], blue: []as any[], total: []as any[]} as any,
		realScoreBySaber: {red: []as any[], blue: []as any[], total: []as any[]} as any,
	};
	...
	for (let i = 0.0; i < accGraphResolution; i += 1.0) {
		...
		let sumsBySaber = {red: [0.0, 0.0], blue: [0.0, 0.0], total: [0.0, 0.0]} as any;
		let dividerBySaber = {red: 0.0, blue: 0.0, total: 0.0} as any;
...
export function processSliceDetails(replay:any) {
	...
	for (let i = 0; i < 12; i++) {
		let mainGridCell = {count: 0, averageScore: 0.0, left: []as any[], right: [] as any[]};
		for (let j = 0; j < 9; j++) {

...
// import { EmptyPromptError, NotImplementedError } from '@/infra/errors'
//
// // user::key-schema
...
//     .setExecutor(async (c) => {
//       if (!c.input) {
//         await c.session.sendQuote(c.session.text('commands.bsbot.config.show.not-implemented'))
//         await c.session.sendQuote(c.session.text(`commands.bsbot.config.all-available-key`))
//         return
//       }
...
//             const f = await c.services.s3?.uploadImgWithUrl(prompt)
//             if (f) {

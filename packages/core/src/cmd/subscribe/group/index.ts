...
import {getEventTypeBySubscriptionType, groupTypes, supportTypes} from "@/cmd/subscribe/types";
...
export const subscribeGroup = async (c: CmdContext<{t?: string}>) => {
  ...
  if(subscription) {
    ...
    subscription = {
      ...
      type: t,
      eventType: getEventTypeBySubscriptionType(t),
      updatedAt: now,
      createdAt: now,
      enabled: true,
      data: {},
  ...
  await c.session.sendQuote(c.session.text(`commands.bsbot.subscribe.success.${t}`))
}

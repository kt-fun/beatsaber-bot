...
export default function BSMapShare({
  ...
  return (
    <>
      <div
        className={
          'w-[400px] rounded-none flex flex-col relative h-auto  my-auto'
        }
        id="render-result"
      >
        <div className={'relative z-10'}>
          <div
            className={'bg-blend-darken bg-black/[.8] text-white  rounded-none'}
          >
            <img
            ...
            <div className="flex w-full h-auto flex-col">
              <div className="p-4">
                <div className="text-ellipsis  line-clamp-2">
                  <span className="text-ellipsis  line-clamp-2 text-xl font-weight bg-gradient-to-r bg-clip-text text-transparent from-red-300 to-blue-300">
                    {bsMap.name}
                  </span>
                </div>
                ...
                <div className="meta  flex space-x-4 text-xs py-2 items-center">
                  ...
                  <div className="flex space-x-1 items-center justify-between">
                    ...
                    <span>
                      {dayjs
                        .duration(bsMap.metadata.duration, 'seconds')
                        .format('mm:ss')}
                    </span>
                  </div>
                  ...
                  <div className="flex space-x-1 items-center justify-between">
                    ...
                    <span className="font-weight bg-gradient-to-r bg-clip-text text-transparent from-red-300 to-blue-300">
                      {bsMap.id}
                    </span>
                  </div>
                ...
                <div className="tags flex flex-wrap justify-start">
                  {bsMap.tags
                    ...
                    ?.map((item) => (
                      <span
                        className="text-xs mx-1 text-white bg-red-500 rounded px-1"
                        key={item}
                      >
                        {getTag(item)}
                      </span>
                ...
                <div className="tags flex flex-wrap justify-start pt-2">
                  {getMods(bsMap)
                    ...
                    ?.map((item) => (
                      <span
                        className="text-xs mx-1 text-white bg-blue-500 rounded px-1"
                        key={item}
                      >
                        {item}
                      </span>
                ...
                <div className="grid grid-cols-2">
                  {bsMap.versions[0].diffs.map((diff) => (
                    <div
                      ...
                      className="text-xs space-x-1 flex items-center"
                    >
                ...
                <p className="text-xs">{bsMap.description}</p>

                <div className={'flex items-center justify-between pt-2'}>
                  <div
                    className={
                      'text-[10px] text-opacity-70 text-slate-100 align-bottom mt-auto mb-0'
                    }
                  >
                    Powered by BSBot
                  </div>
          ...
          <img
            src={bg}
            className={'inset-0 absolute h-full object-cover -z-10'}
            loading={'eager'}
            alt={'random background img'}
          />
        </div>
      </div>
    </>

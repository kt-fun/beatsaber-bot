// export function checkBSOR(file, isLink, completion) {
// 	if (isLink) {
...
export function checkBSORFile(file, completion) {
...
export function decode(arrayBuffer, completion) {
  const {ok, replay} = _decode(arrayBuffer)
  if (!ok)  completion('Error: failed to decode replay')
  completion(replay)
}

export async function decodeAsync(arrayBuffer) {
  const {ok, replay} = _decode(arrayBuffer)
  if (!ok) throw new Error("failed to decode replay, not a target function")
  return replay
}
export function DecodeInfo(dataView) {
  var result = {}
...
export function DecodeNotes(dataView) {
  const length = DecodeInt(dataView)
  var result = []
  for (var i = 0; i < length; i++) {
...
function DecodeNote(dataView) {
  ...
  if (
    result.eventType == NoteEventType.good ||
    result.eventType == NoteEventType.bad
  ) {
...
function DecodeString(dataView) {
  ...
  const string = enc.decode(
    new Int8Array(
      dataView.buffer.slice(dataView.pointer + 4, length + dataView.pointer + 4)
    )
  )
...
function DecodeName(dataView) {
  ...
  if (length > 0) {
    while (
      dataView.getInt32(length + dataView.pointer + 4 + lengthOffset, true) !=
        6 &&
      dataView.getInt32(length + dataView.pointer + 4 + lengthOffset, true) !=
        5 &&
  ...
  const string = enc.decode(
    new Int8Array(
      dataView.buffer.slice(
        dataView.pointer + 4,
        length + dataView.pointer + 4 + lengthOffset
      )
    )
  )
